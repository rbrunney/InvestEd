            ...
            DefaultConsumer consumer = new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) {
                    ...
                    if (OrderConsumerService.isTradingHours()) {
                        ...
                        if(OrderConsumerService.hasHitSellLimit(Double.parseDouble(msgToMap.get("stop-loss-price")), currentPrice)) {
                            OrderConsumerService.putIntoQueue(channel, "order.limit-order", new HashMap<>(){{
                                ...
                                put("limit-price", msgToMap.get("limit-price"));
                            }}.toString().getBytes());
                        } else {
                            OrderConsumerService.putIntoQueue(channel, "order.stop-price-order", body);
                        }
                    } else {
                        OrderConsumerService.putIntoQueue(channel, "order.stop-price-order", body);
                    }
